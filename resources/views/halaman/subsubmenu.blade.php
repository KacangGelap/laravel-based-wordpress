...
@section('content')
            ...
            <div class="d-md-flex card-header justify-content-between">
                <div>
                    ...
                    <div class="container fw-bold text-secondary">
                        ...
                        <span class="text-capitalize">{{ strtolower($submenu->menu->menu) }}</span>
                        <span class="mx-1">></span>
                        <a href="{{ route('submenu.index', $submenu->menu->id) }}" class="text-dark text-decoration-none">Sub-Menu</a>
                        <span class="mx-1">></span>
                        <span class="text-capitalize">{{ strtolower($submenu->sub_menu) }}</span>
                        <span class="mx-1">></span>
                        <a href="{{ route('subsubmenu.index', $submenu->id) }}" class="text-dark text-decoration-none">Sub-Sub-Menu</a>
                    </div>
                </div>
                <div>
                    <a href="{{route('submenu.index', $submenu->menu->id)}}" class="btn btn-dark">Kembali</a>
                </div>
            </div>
            <div class="card-body">
                <div class="">
                    ...
                    <div class="d-md-flex">
                        ...
                        @if($isExists === false && strcasecmp($submenu->menu->menu, "tentang") == 0)<a href="{{route('subsubmenu.create',['submenu' => $submenu->id, 'tambah' => 3])}}" class="badge text-decoration-none text-bg-success mx-1"><i class="bi-diagram-3 me-1"></i>Identitas PD/UPT</a>@endif
                        <a href="{{route('subsubmenu.create',['submenu' => $submenu->id, 'tambah' => 4])}}" class="badge text-decoration-none text-bg-info mx-1"><i class="bi-globe me-1"></i>Link Medsos / GForm</a>
                    </div>
                </div>
                
                <hr class="mb-2">
                <div class="">
                    <p class="fw-bold">Tambah Sub-Sub-Menu Bertingkat</p>
                    <a href="{{route('subsubmenu.create',['submenu' => $submenu->id, 'tambah' => 5])}}" class="badge text-decoration-none text-bg-dark mx-1"><i class="bi-grid-1x2 me-1"></i>Dropdown</a>
                </div>
                <hr>
                        ...
                        <tbody>
                            @foreach ($subsubmenu as $item)
                                <tr>
                                    ...
                                    <td class="text-sm-start" style="font-size: 12px">{{ $item->sub_sub_menu }}</td>
                                    <td style="font-size: 12px"> {{$item->type}}</td>
                                    <td style="font-size: 12px">
                                        @if($item->type == 'dropdown')
                                            Jumlah Sub-Sub-Sub-Menu : {{$item->subSubSubMenus->count()}}
    ...
    @foreach ($subsubmenu as $item)
        <form id="hapus-{{ $item->id }}" action="{{ route('subsubmenu.delete', ['submenu'=>$item->submenu->id, 'subsubmenu'=>$item->id]) }}" method="POST" class="d-none">
            @csrf
